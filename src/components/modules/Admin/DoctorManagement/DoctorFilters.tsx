...
const DoctorFilters = ({ specialties }: IDoctorFilterProps) => {
  ...
  const [appointmentFeeInput, setAppointmentFeeInput] = useState(
    () => searchParams.get("appointmentFee") || "",
  );
  ...
  const debouncedAppointmentFee = useDebounce(appointmentFeeInput, 500);
  ...
  useEffect(() => {
    ...
    if (debouncedAppointmentFee) {
      params.set("appointmentFee", debouncedAppointmentFee);
    } else {
